...
for i in range(original_students[0][0]+1): # 매칭을 0부터 a반 남학생 숫자까지 함.(b반 여자애들이랑)
    ...
    if backtracking(i, 1):
        break
    # matching_candidate[0][1] = \
    x = original_students[0][0] - i # a남자 -> c여자
    if backtracking(x, 2):
        continue
    for j in range(original_students[1][0]+1):
        ...
        if backtracking(j, 0):
            break
        # matching_candidate[1][1] = \
        y = original_students[1][0] - j # b남자 ->c여자
        if backtracking(y, 2):
            break
        if x + y != original_students[2][1]:
            continue
        for k in range(original_students[2][0]+1):
            # matching_candidate[2][0] = k # c남자 -> a여자
            if backtracking(k, 0):
                break
            if j + k != original_students[0][1]:
                continue
            # matching_candidate[2][1] = \
            z = original_students[2][0] - k # c남자 -> b여자
            if backtracking(z, 1):
                continue
            if [j+k, i+z, x+y] == for_test:
                done = 1
                matching_candidate = [[i,x],[j,y],[k,z]]
                print(done)
